...
# to 0-1 range
train_dataset = datasets.CIFAR10(root='dataC', 
                                 train=True, 
                                 transform=transform,
                                 download=True)

test_dataset = datasets.CIFAR10(root='dataC', 
                                train=False, 
                                transform=transform)
...
#     class_report = classification_report(true_labels, predicted_labels, target_names=[f"Class {i}" for i in range(10)], digits = 4)
...
#         # Generate adversarial examples
#         # Generate adversarial examples
#         x_test_adv = attack.generate(x=images.cpu().detach().numpy())  # ART needs numpy arrays
...
#         logits_noised = torch.from_numpy(logits_noised_np).to(device)
...
# print(classification_report(y_true, y_pred_confident, target_names=target_names, zero_division=0,digits=4))
...
##############################
### GAUSSIAN NOISE as PreProcessing for Testing#######
#############################
...
print("The followinf]g is about using gauassina noise before predicting the output")
...
# Generate classification report
target_names = [f"Class {i}" for i in range(NUM_CLASSES)]
classification_report_str = classification_report(y_true, y_pred, target_names=target_names, digits=4)
print("Classification report after applying gaussain noise on clean test images")
print(classification_report_str)
