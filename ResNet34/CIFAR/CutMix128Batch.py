...
print('Accuracy on clean test images with CutMix training and Gaussian postprocessor: ')
print(classification_report_str)

# Calculate and print inference speed per 1000 images
total_images = len(y_true)
time_per_thousand = (total_inference_time / total_images) * 1000 if total_images > 0 else 0
print(f"Inference speed on CUTMIX -Guassian without any attack: {time_per_thousand:.2f} milliseconds per 1000 images")
...
print(f"FGSM Attack Accuracy with CUTMIX-Gaussian Noise: {accuracy:.2f}%")
print("FGSM Classification Report with CUTMIX-Gaussian Noise:\n", report)
...
print(f"PGD Attack Accuracy with CUTMIX-Gaussian Noise: {accuracy:.2f}%")
print("PGD Classification Report with CUTMIX-Gaussian Noise:\n", report)
...
print(f"FGSM Attack Accuracy with CUTMIX-High Confidence: {accuracy:.2f}%")
print("FGSM Classification Report with CUTMIX - High Confidence:\n", report)
print(f"FGSM Inference Speed: {speed:.2f} ms per 1000 images")
...
print(f"PGD Attack Accuracy with CUTMIX - High Confidence: {accuracy:.2f}%")
print("PGD Classification Report with CUTMIX - High Confidence:\n", report)
print(f"PGD Inference Speed: {speed:.2f} ms per 1000 images")
...
def evaluate_with_reverse_sigmoid(model, data_loader, device):
    ...
    print(f'Accuracy on CLean Test IMages with CUTMIX - Reverse Sigmoid postprocessing: {accuracy:.2f}%')
...
def evaluate_model_with_reverse_sigmoid_and_attack(model, data_loader, device, attack, reverse_sigmoid_params={'beta': 1.0, 'gamma': 0.1}):
    ...
    accuracy = (correct_pred / num_examples) * 100 if num_examples > 0 else 0
    print(f'Accuracy with {attack.__class__.__name__} and CUTMIX - Reverse Sigmoid defense: {accuracy:.2f}%')
